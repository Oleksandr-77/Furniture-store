...
function requestLocalProd() {
  ...
  if (
    localStorage.getItem("listLocalProduct") != null &&
    localStorage.getItem("listLocalProduct") != "[]"
  ) {
    ...
    for (let i of parseListProd) {
      document
        .querySelector(".cart__product-list")
    ...
    document.querySelector(".cart__quantity-numb").innerHTML =
      document.querySelector(".cart__product-list").children.length;
...
document.querySelector(".products__content").onclick = function (event) {
  if (event.target.classList.contains("products__item-button")) {
    if (!event.target.classList.contains("_active")) {
      ...
      let cartProdItem = `<li class='cart__pruduct-item cart__pruduct-item${
        event.target.dataset.prod
      }'>
                        <img src='ICON/PRODUCTS__IMG${
                          event.target.dataset.prod
                        }.jpg' class='cart__pruduct-img'>
                        <button class='cart__pruduct-delete' data-delete='cart__pruduct-item${
                          event.target.dataset.prod
                        }' data-stopper='products__item${
        event.target.dataset.prod
      }-button'></button>
                        <div class='cart__pruduct-box'>
                           <p class='cart__pruduct-name'>${
                             document.querySelector(
                               `.products__item${event.target.dataset.prod}-name`
                             ).innerHTML
                           }</p>
                           <p class='cart__pruduct-price'>${
                             document.querySelector(
                               `.products__item${event.target.dataset.prod}-price`
                             ).innerHTML
                           }</p>
                        </div>
                     </li>`;
      ...
      document
        .querySelector(".cart__product-list")
        .insertAdjacentHTML("beforeend", cartProdItem);

      // добавляем копию товара из корзины в localStorage
      localStorage.setItem(
        `cart__pruduct-item${event.target.dataset.prod}`,
        cartProdItem
      );
      ...
      if (localStorage.getItem("listLocalProduct") == null) {
        ...
        localStorage.setItem(
          "listLocalProduct",
          JSON.stringify(listLocalProduct)
        );
        ...
        localStorage.setItem(
          "listLocalProduct",
          JSON.stringify(parseListLocalProduct)
        );
      }
...
document.querySelector(".cart__product-list").onclick = function (event) {
  if (event.target.classList.contains("cart__pruduct-delete")) {
    ...
    document
      .querySelector(`.${event.target.dataset.stopper}`)
    ...
    if (localStorage.getItem("listLocalProduct") != null) {
      ...
      for (let i of parseListLocalProduct) {
        ...
        if (i == event.target.dataset.delete) {
          parseListLocalProduct.splice(
            parseListLocalProduct.indexOf(event.target.dataset.delete),
            1
          );
        }
      }
...
document.querySelector(".cart__but-close").onclick = function (event) {
  document.querySelector(".cart__content").style.transform =
    "translate(480px, 0)";
  document.querySelector(".cart__stopper").style.transform =
    "translate(480px, 0)";
  setTimeout(() => {
...
document.querySelector(".header-menu__cart").onclick = function () {
  if (
    !document.querySelector(".header-menu__cart").classList.contains("_active")
  ) {
    ...
    document.querySelector(".header__cart").onclick = function (event) {
      if (event.target.getAttribute("class") == "header__cart") {
        document
          .querySelector(".header-menu__cart")
          .classList.remove("_active");
        document.querySelector(".cart__content").style.transform =
          "translate(480px, 0)";
        document.querySelector(".cart__stopper").style.transform =
...
document.querySelector(".header__burger-icon").onclick = function () {
  if (
    !document
      .querySelector(".header__burger-icon")
    ...
    setTimeout(() => {
      ...
      document.querySelector(".burger__stopper").style.transform =
        "translate(0, 0)";
      document.querySelector(".burger__content").style.transform =
        "translate(0, 0)";
      document.querySelector(".header-menu__nav").style.transform =
        "translate(0, 0)";
      document.querySelector(".header-menu__icons").style.transform =
        "translate(0, 0)";
      if (document.body.clientWidth <= 487) {
        document.querySelector(".header-menu__search").style.transform =
          "translate(0, 0)";
      }
    }, 10);
    document.body.style.overflow = "hidden";
    ...
    document.querySelector(".header__burger").onclick = function (event) {
      if (event.target.getAttribute("class") == "header__burger") {
        ...
        setTimeout(() => {
          ...
          setTimeout(
            () =>
              (document.querySelector(".header__burger").style.display =
                "none"),
            300
          );
        }, 300);
    ...
    document.querySelector(".header__burger-icon").classList.contains("_active")
  ) {
...
document
  .querySelector(".header-menu__products")
  .addEventListener("touchstart", () => {
    if (
      document
        .querySelector(".header-menu__products-link")
        .getAttribute("href") == "https://yandex.by/"
    ) {
      document
        .querySelector(".header-menu__products-link")
        .setAttribute("onclick", "true");
    } else if (
      document
        .querySelector(".header-menu__products-link")
        .getAttribute("href") != "https://yandex.by/"
      ...
      document
        .querySelector(".header-menu__products-link")
        .setAttribute("href", "https://yandex.by/");
    }

    function refreshProductsLink() {
      if (
        window.getComputedStyle(document.querySelector(".products-dropdown"))
          .display == "none"
      ) {
        document
          .querySelector(".header-menu__products-link")
...
document
  .querySelector(".header-menu__rooms")
  .addEventListener("touchstart", () => {
    //Второе нажатие. Разрешает клик, тем самым совершается переход по ссылке.
    if (
      document.querySelector(".header-menu__rooms-link").getAttribute("href") ==
      "https://yandex.by/"
    ) {
      document
        .querySelector(".header-menu__rooms-link")
        .setAttribute("onclick", "true");
    ...
    function refreshRoomsLink() {
      if (
        window.getComputedStyle(document.querySelector(".rooms-dropdown"))
          .display == "none"
      ) {
        document
          .querySelector(".header-menu__rooms-link")
...
window.addEventListener("resize", () => {
  ...
  if (document.body.clientWidth < 768) {
    if (window.innerHeight < 370) {
      document.querySelector(".header-menu__list").style.height =
